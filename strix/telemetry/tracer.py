...
class Tracer:
    def __init__(self, run_name: str | None = None):
        ...
        self.active_poc_batches: dict[str, dict[str, Any]] = {}
        self.recent_poc_executions: list[dict[str, Any]] = []
        self.poc_summary_by_agent: dict[str, dict[str, int]] = {}
...
"""
    ...
    def _bump_poc_summary(self, agent_id: str, status: str) -> None:
        summary = self.poc_summary_by_agent.setdefault(agent_id, {})
        summary[status] = summary.get(status, 0) + 1

    def record_poc_batch_disclosed(
        self,
        agent_id: str,
        target: str,
        cursor: int,
        ...
        component = ""
        if candidates:
            component = str(candidates[0].get("component", ""))

        self.active_poc_batches[agent_id] = {
            "target": target,
            "component": component,
            "cursor": cursor,
        ...
        for candidate in candidates:
            status = str(candidate.get("status", "disclosed"))
            self._bump_poc_summary(agent_id, status)

        event_type = "poc.batch.advanced" if cursor else "poc.batch.disclosed"
        self._emit_event(
            event_type,
    ...
    def record_poc_execution_started(
        self,
        agent_id: str,
        target: str,
        ...
        self._emit_event(
            ...
            actor={"agent_id": agent_id},
            payload={"target": target, "candidate_ids": candidate_ids},
            status="running",
            source="strix.pocs",
        )

    def record_poc_execution_completed(
        self,
        agent_id: str,
        target: str,
        ...
        for result in results:
            status = str(result.get("status", "executed_miss"))
            self._bump_poc_summary(agent_id, status)
            self.recent_poc_executions.insert(
                0,
                {
                    "agent_id": agent_id,
                    "target": target,
        ...
        self.recent_poc_executions = self.recent_poc_executions[:25]

        self._emit_event(
            "poc.execution.completed",
            actor={"agent_id": agent_id},
            payload={"target": target, "results": results, "skipped": skipped},
            status="completed",
            source="strix.pocs",
        )

    def record_poc_execution_skipped(
        self,
        agent_id: str,
        target: str,
        skipped: list[dict[str, Any]],
        ...
        self._emit_event(
            "poc.execution.skipped",
            actor={"agent_id": agent_id},
            payload={"target": target, "skipped": skipped},
            status="completed",
            source="strix.pocs",
        )
