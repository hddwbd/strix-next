...
class DockerRuntime(AbstractRuntime):
    def __init__(self) -> None:
        ...
        self._tool_server_ready = False
        self._sandbox_lock = threading.RLock()

    def _reset_tool_server_state(self, *, clear_container: bool = False) -> None:
        if clear_container:
    ...
    def _wait_for_tool_server(
        self, max_retries: int = 30, timeout: int = 5, initial_delay: int = 5
    ) -> None:
        if self._tool_server_port is None:
            raise RuntimeError("Tool server port not initialized")
        ...
        if self._scan_container:
            try:
                ...
                if self._scan_container.status == "running":
                    if not self._tool_server_ready:
                        self._wait_for_tool_server(initial_delay=0)
                    return self._scan_container
            except NotFound:
                self._reset_tool_server_state(clear_container=True)
